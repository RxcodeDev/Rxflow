 ...
 * Embeddable rich-text field for forms (no submit button).
 * Supports: slash commands (/), image paste/upload, @mentions, bold, italic, lists, etc.
 * Calls onChange(jsonString | null) on every keystroke.
 */
...
export interface FormDescriptionEditorProps {
  /** Called on every content change with JSON.stringify(tiptapDoc) or null if empty */
...
function MentionMenuFloating({
  ...
  x: number; y: number;
  items: UserOpt[]; activeIndex: number;
  ...
  const finalY = typeof window !== 'undefined' && y + menuH > window.innerHeight ? y - menuH - 24 : y;
  const finalX = typeof window !== 'undefined' ? Math.min(x, window.innerWidth - 232) : x;
...
export default function FormDescriptionEditor({
  onChange,
  users = [],
  placeholder = 'Agrega una descripción… usa / para formato, pega imágenes o @ para mencionar',
}: FormDescriptionEditorProps) {
  ...
  const editor = useEditor({
    ...
    editorProps: {
      ...
      handleKeyDown(_view, event) {
        ...
        if (mMenu && mMenu.items.length > 0) {
          if (event.key === 'ArrowDown') { setMention({ ...mMenu, activeIndex: (mMenu.activeIndex + 1) % mMenu.items.length }); return true; }
          if (event.key === 'ArrowUp')   { setMention({ ...mMenu, activeIndex: (mMenu.activeIndex - 1 + mMenu.items.length) % mMenu.items.length }); return true; }
          if (event.key === 'Enter')     { const u = mMenu.items[mMenu.activeIndex]; if (u) insertMention(u); return true; }
          if (event.key === 'Escape')    { setMention(null); return true; }
        }
        // Slash menu
        const menu = slashRef.current;
        if (menu) {
          if (event.key === 'ArrowDown') { const next = { ...menu, activeIndex: (menu.activeIndex + 1) % menu.items.length }; slashRef.current = next; setSlashMenuState(next); return true; }
          if (event.key === 'ArrowUp')   { const prev = { ...menu, activeIndex: (menu.activeIndex - 1 + menu.items.length) % menu.items.length }; slashRef.current = prev; setSlashMenuState(prev); return true; }
          if (event.key === 'Enter')     { const item = menu.items[menu.activeIndex]; if (item) executeSlash(item); return true; }
    ...
    onUpdate({ editor: e }) {
      // Notify parent
      onChange(e.isEmpty ? null : JSON.stringify(e.getJSON()));
      ...
      if (!slashMatch) {
        ...
        const filtered = SLASH_COMMANDS.filter(cmd =>
          query === '' || cmd.label.toLowerCase().includes(query) || cmd.keywords.some(k => k.includes(query)),
        );
        const safePos = Math.min(state.selection.from, state.doc.content.size - 1);
        const coords = e.view.coordsAtPos(safePos);
        setSlash(filtered.length > 0 ? { x: coords.left, y: coords.bottom + 6, activeIndex: 0, items: filtered } : null);
      }
      ...
      if (!mentionMatch) {
        ...
        const filtered = users.filter(u => query === '' || u.name.toLowerCase().includes(query));
        const safePos2 = Math.min(state.selection.from, state.doc.content.size - 1);
        const coords2 = e.view.coordsAtPos(safePos2);
        setMention({ x: coords2.left, y: coords2.bottom + 6, query, activeIndex: 0, items: filtered });
      }
    },
  ...
  return (
    <div
      className={[
        'w-full border rounded-[0.625rem] bg-[var(--c-bg)]',
