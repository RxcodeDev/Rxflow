...
// ── CycleCard ───────────────────────────────────────────────────────────────
function CycleCard({
  cycle, isExpanded, tasks, loadingTasks,
  onToggle, onManage, onEdit, onDelete, onStatusChange, onTaskClick, onRemoveTask,
}: {
  cycle: CycleSummary;
  isExpanded: boolean;
  tasks: TaskItem[];
  ...
  return (
    <div className="rounded-xl border border-[var(--c-border)] bg-[var(--c-bg)] transition-shadow hover:shadow-sm"
      style={{ borderLeft: `3px solid var(--c-cycle-${sk})` }}>
      <div className="px-4 pt-4 pb-3 cursor-pointer hover:bg-[var(--c-hover)] transition-colors"
        onClick={onToggle} role="button" aria-expanded={isExpanded}>
          ...
          <div className="flex items-center gap-2 shrink-0">
            <div className="relative">
              <button type="button"
                ...
                style={{ color: `var(--c-cycle-${sk})`, background: `var(--c-cycle-${sk}-bg)` }}
              ...
              {statusOpen && (
                <>
                  ...
                  <div className="absolute left-0 top-full mt-1 z-20 min-w-[10rem] rounded-xl border border-[var(--c-border)] bg-[var(--c-bg)] shadow-lg overflow-hidden py-1"
                    ...
                    {STATUSES.map(s => {
                      ...
                      return (
                        <button key={s.value} type="button"
                          ...
                          <span className="w-2 h-2 rounded-full shrink-0" style={{ background: `var(--c-cycle-${ssk})` }} />
            ...
            <div className="relative">
              <button type="button" aria-label="Opciones del cycle"
              ...
              {menuOpen && (
                <>
                  ...
                  <div className="absolute right-0 top-full mt-1 z-20 min-w-[9rem] rounded-xl border border-[var(--c-border)] bg-[var(--c-bg)] shadow-lg overflow-hidden py-1"
                    onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex items-center gap-2 mt-2.5 flex-wrap">
          ...
          <span className="text-[12px] text-[var(--c-text-sub)]">{formatDate(cycle.start_date)} → {formatDate(cycle.end_date)}</span>
          {cycle.days_left !== null && cycle.status === 'activo' && (
            <span className="ml-auto text-[11px] font-semibold rounded-full px-2 py-0.5"
        ...
        <div className="mt-3">
          <div className="flex justify-between mb-1.5">
            ...
            <span className="text-[11px] font-semibold text-[var(--c-text-sub)] tabular-nums">{cycle.tasks_done}/{cycle.tasks_total} · {pct}%</span>
          </div>
          <div className="h-1.5 rounded-full bg-[var(--c-border)] overflow-hidden">
            <div className="h-full rounded-full transition-all" style={{ width: `${pct}%`, background: `var(--c-cycle-${sk})` }} />
          </div>
        </div>
      </div>
      ...
      {isExpanded && (
        <div className="border-t border-[var(--c-border)] bg-[var(--c-hover)] rounded-b-xl overflow-hidden">
          {loadingTasks ? (
            <div className="flex flex-col gap-2 p-3">{[1,2,3].map(i => <Skeleton key={i} className="h-8 rounded-lg" />)}</div>
          ) : tasks.length === 0 ? (
            ...
            <ul className="divide-y divide-[var(--c-line)]">
              {tasks.map(t => (
                <li key={t.id} className="flex items-center group">
                  ...
                  <button type="button" aria-label="Quitar del cycle"
          ...
          <div className="px-4 py-2 border-t border-[var(--c-line)]">
            <button type="button" onClick={(e) => { e.stopPropagation(); onManage(); }}
...
// ── Timeline view ───────────────────────────────────────────────────────────
function TimelineView({ cycles, onCycleClick }: { cycles: CycleSummary[]; onCycleClick: (c: CycleSummary) => void }) {
  ...
  while (m <= rangeEnd) {
    const x = dayOffset(m);
    if (x >= 0 && x <= chartW) months.push({ label: m.toLocaleDateString('es', { month: 'short', year: '2-digit' }), x });
    m.setMonth(m.getMonth() + 1);
  }

  const todayX  = dayOffset(today);
  const showToday = todayX >= 0 && todayX <= chartW;
  const groups  = STATUS_ORDER.map(s => ({ status: s, list: datedCycles.filter(c => c.status === s) })).filter(g => g.list.length > 0);
      ...
      <div className="overflow-x-auto">
        <div style={{ minWidth: `${LABEL_W + chartW + 32}px` }}>
          <div className="flex border-b border-[var(--c-border)] bg-[var(--c-hover)]">
            <div className="shrink-0 border-r border-[var(--c-border)] flex items-end px-4 pb-2" style={{ width: LABEL_W }}>
            ...
            <div className="flex-1 relative" style={{ height: HEADER_H }}>
              {months.map(mo => (
                <div key={mo.label + mo.x} className="absolute bottom-2 flex flex-col items-start gap-0.5" style={{ left: mo.x + 16 }}>
                  <span className="text-[11px] font-semibold text-[var(--c-text-sub)] uppercase tracking-wide capitalize whitespace-nowrap">{mo.label}</span>
                </div>
              ))}
              {showToday && (
                <div className="absolute bottom-1 flex flex-col items-center" style={{ left: todayX + 16, transform: 'translateX(-50%)' }}>
          ...
          <div className="relative">
            <div className="absolute inset-0 pointer-events-none" style={{ left: LABEL_W + 16 }}>
              {months.map(mo => (
                <div key={'line-' + mo.x} className="absolute top-0 bottom-0 w-px bg-[var(--c-line)]" style={{ left: mo.x }} />
              ))}
              {showToday && (
                <div className="absolute top-0 bottom-0 w-0.5" style={{ left: todayX, background: 'var(--c-danger)', opacity: 0.35 }} />
              )}
            </div>
                  ...
                  {group.list.map((c, ri) => {
                    ...
                    return (
                      <div key={c.id}
                        ...
                        style={{ height: ROW_H }}>
                        <div className="shrink-0 border-r border-[var(--c-border)] h-full flex flex-col justify-center px-4 gap-0.5" style={{ width: LABEL_W }}>
                        ...
                        <div className="flex-1 relative h-full" style={{ paddingLeft: 16, paddingRight: 16 }}>
                          <button type="button" onClick={() => onCycleClick(c)}
                            ...
                            <div className="absolute inset-0" style={{ background: `var(--c-cycle-${csk}-bg)`, border: `1.5px solid var(--c-cycle-${csk})`, borderRadius: 8, opacity: 0.7 }} />
                            <div className="absolute inset-y-0 left-0 rounded-l-lg" style={{ width: `${pct}%`, background: `var(--c-cycle-${csk})`, opacity: 0.5 }} />
                            {barW > 48 && (
                              <span className="relative z-10 px-2 text-[10px] font-bold leading-none flex items-center h-full gap-1 whitespace-nowrap" style={{ color: `var(--c-cycle-${csk})` }}>
                                {pct}%
                              </span>
                            )}
                          </button>
...
function CalendarView({ cycles }: { cycles: CycleSummary[] }) {
  ...
  function cyclesForDay(d: number): CycleSummary[] {
    ...
    return datedCycles.filter(c => { const s = toDay(c.start_date!); const e = toDay(c.end_date!); return day >= s && day <= e; });
  }
  ...
  const visibleCycles = datedCycles.filter(c => {
    const s = toDay(c.start_date!); const e = toDay(c.end_date!);
    return s <= new Date(year, month, lastDay) && e >= new Date(year, month, 1);
  });
    ...
    <div className="flex flex-col gap-5">
      <div className="flex items-center justify-between">
        <button type="button" onClick={() => setViewDate(new Date(year, month - 1, 1))}
          className="p-2 rounded-xl hover:bg-[var(--c-hover)] transition-colors bg-transparent border border-[var(--c-border)] cursor-pointer" aria-label="Mes anterior">
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" aria-hidden="true" className="text-[var(--c-text-sub)]"><path d="M15 18l-6-6 6-6" /></svg>
        </button>
        <h2 className="text-base font-bold text-[var(--c-text)] capitalize">{monthLabel}</h2>
        <button type="button" onClick={() => setViewDate(new Date(year, month + 1, 1))}
          className="p-2 rounded-xl hover:bg-[var(--c-hover)] transition-colors bg-transparent border border-[var(--c-border)] cursor-pointer" aria-label="Mes siguiente">
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" aria-hidden="true" className="text-[var(--c-text-sub)]"><path d="M9 18l6-6-6-6" /></svg>
        </button>
      </div>
        ...
        <div className="flex flex-wrap gap-x-4 gap-y-1.5">
          {visibleCycles.slice(0, 8).map(c => (
            <div key={c.id} className="flex items-center gap-1.5">
              <span className="w-2 h-2 rounded-full shrink-0" style={{ background: `var(--c-cycle-${sKey(c.status)})` }} />
              <span className="text-[11px] text-[var(--c-text-sub)]">{c.name}</span>
            </div>
          ))}
          {visibleCycles.length > 8 && <span className="text-[11px] text-[var(--c-muted)]">+{visibleCycles.length - 8} más</span>}
        </div>
      )}
      <div className="rounded-2xl border border-[var(--c-border)] overflow-hidden">
        <div className="grid grid-cols-7 border-b border-[var(--c-border)] bg-[var(--c-hover)]">
          {['Lun','Mar','Mié','Jue','Vie','Sáb','Dom'].map((day, i) => (
            <div key={day} className={`py-3 text-center text-[11px] font-semibold uppercase tracking-wider ${i >= 5 ? 'text-[var(--c-muted)]' : 'text-[var(--c-text-sub)]'}`}>{day}</div>
          ))}
        </div>
        {Array.from({ length: cells.length / 7 }, (_, week) => (
          <div key={week} className={`grid grid-cols-7 ${week < cells.length / 7 - 1 ? 'border-b border-[var(--c-border)]' : ''}`}>
            {cells.slice(week * 7, week * 7 + 7).map((d, col) => {
              ...
              return (
                <div key={i} className={`min-h-[7rem] flex flex-col ${col < 6 ? 'border-r border-[var(--c-border)]' : ''} ${d === null ? 'bg-[var(--c-hover)]' : ''}`}>
                  {d !== null && (
                    <>
                      <div className="px-2.5 pt-2.5 pb-1.5">
                        <span className={`inline-flex items-center justify-center w-7 h-7 rounded-full text-[13px] font-semibold leading-none select-none ${isToday ? 'bg-[var(--c-text)] text-[var(--c-bg)]' : isWeekend ? 'text-[var(--c-muted)]' : 'text-[var(--c-text-sub)]'}`}>{d}</span>
                      </div>
                        ...
                        {shown.map(c => {
                          ...
                          return (
                            <div key={c.id} title={c.name}
                              ...
                              style={{ background: `var(--c-cycle-${sk}-bg)`, color: `var(--c-cycle-${sk})` }}>
                              {showLabel(c, d) ? c.name : '\u00a0'}
                            </div>
                          );
                        })}
                        {overflow > 0 && <p className="text-[10px] text-[var(--c-muted)] px-1 font-medium">+{overflow} más</p>}
                      </div>
                    </>
                  )}
                </div>
...
export default function ProjectCyclesPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [form,       setForm]       = useState({ name: '', status: 'planificado' as CycleSummary['status'], start_date: '', end_date: '' });
  ...
  // Cycles for this project only
  const cycles = useMemo(() => allCycles.filter(c => c.project_code === code), [allCycles, code]);

  const filteredCycles = useMemo(() =>
    filterStatus.length > 0 ? cycles.filter(c => filterStatus.includes(c.status)) : cycles,
  [cycles, filterStatus]);

  const filteredGroups = useMemo(() =>
    STATUS_ORDER.map(s => ({ status: s, cycles: filteredCycles.filter(c => c.status === s) })).filter(g => g.cycles.length > 0),
  [filteredCycles]);
  ...
  function toggleCycle(cycleId: string) {
    if (expandedId === cycleId) { setExpandedId(null); return; }
    setExpandedId(cycleId);
    if (cycleTasks[cycleId]) return;
  ...
  async function addTaskToCycle(cycleId: string, task: TaskItem) {
    ...
    setCycleTasks(prev => ({ ...prev, [cycleId]: [...(prev[cycleId] ?? []), task] }));
  }
  ...
  async function removeTaskFromCycle(cycleId: string, taskId: string) {
    ...
    setCycleTasks(prev => ({ ...prev, [cycleId]: (prev[cycleId] ?? []).filter(t => t.id !== taskId) }));
    playDelete();
  }
  ...
  async function addEpicToCycle(cycleId: string, epicId: string, projectCode: string) {
    ...
    apiGet<ApiWrapped<TaskItem[]>>(`/tasks?cycleId=${cycleId}`)
      .then(res => setCycleTasks(prev => ({ ...prev, [cycleId]: res.data }))).catch(console.error);
    apiGet<ApiWrapped<TaskItem[]>>(`/projects/${projectCode}/tasks`)
      .then(res => setAllProjTasks(res.data)).catch(console.error);
  }

  async function handleCreate(e: React.FormEvent) {
    e.preventDefault();
    if (!form.name.trim()) { setFormError('El nombre es requerido'); return; }
    if (form.start_date && form.end_date && form.end_date <= form.start_date) { setFormError('La fecha de fin debe ser posterior a la de inicio'); return; }
    if ((form.start_date && !form.end_date) || (!form.start_date && form.end_date)) { setFormError('Debes indicar ambas fechas o ninguna'); return; }
    setSaving(true); setFormError('');
    try {
      const payload: Record<string, string | null> = { project_code: code, name: form.name.trim(), status: form.status };
      if (form.start_date && form.end_date) { payload.start_date = form.start_date; payload.end_date = form.end_date; }
  ...
  function openEditModal(cycle: CycleSummary) {
    setEditCycle(cycle);
    setEditForm({ name: cycle.name, status: cycle.status as CycleSummary['status'], start_date: cycle.start_date ? cycle.start_date.slice(0, 10) : '', end_date: cycle.end_date ? cycle.end_date.slice(0, 10) : '' });
    setEditError('');
  }
  ...
  async function handleEdit(e: React.FormEvent) {
    ...
    if (editForm.start_date && editForm.end_date && editForm.end_date <= editForm.start_date) { setEditError('La fecha de fin debe ser posterior a la de inicio'); return; }
    setEditSaving(true); setEditError('');
    try {
      await apiPatch(`/cycles/${editCycle.id}`, { name: editForm.name.trim(), status: editForm.status, start_date: editForm.start_date || null, end_date: editForm.end_date || null });
      playSuccess();
      setEditCycle(null);
      fetchData();
    } catch (err: unknown) {
  ...
  async function handleDelete() {
    ...
    setAllCycles(prev => prev.filter(c => c.id !== id));
    setCycleTasks(prev => { const n = { ...prev }; delete n[id]; return n; });
  }

  const viewButtons: { id: ViewMode; label: string; icon: React.ReactNode }[] = [
    { id: 'list', label: 'Lista', icon: <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden="true"><line x1="8" y1="6" x2="21" y2="6"/><line x1="8" y1="12" x2="21" y2="12"/><line x1="8" y1="18" x2="21" y2="18"/><line x1="3" y1="6" x2="3.01" y2="6"/><line x1="3" y1="12" x2="3.01" y2="12"/><line x1="3" y1="18" x2="3.01" y2="18"/></svg> },
    { id: 'timeline', label: 'Línea de tiempo', icon: <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden="true"><line x1="3" y1="12" x2="21" y2="12"/><polyline points="8 8 3 12 8 16"/><polyline points="16 8 21 12 16 16"/></svg> },
    { id: 'calendar', label: 'Calendario', icon: <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden="true"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg> },
  ];
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <nav className="flex items-center gap-1.5 text-[13px] text-[var(--c-muted)]" aria-label="Ruta">
        ...
        {loading ? <Skeleton className="w-24 h-4" /> : <span className="text-[var(--c-text-sub)] font-medium">{project?.name ?? code}</span>}
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden="true"><path d="M9 18l6-6-6-6"/></svg>
        <span className="text-[var(--c-text)]">Cycles</span>
      </nav>

      {/* View tabs */}
      <ProjectViewTabs projectCode={projectId} active="cycles" />

      {/* Header */}
      <div className="flex items-center gap-2 flex-wrap">
        <h1 className="text-2xl font-bold text-[var(--c-text)] shrink-0 mr-auto">
          Cycles
          {project && <span className="ml-2 text-base font-normal text-[var(--c-text-sub)]">— {project.name}</span>}
        </h1>
        ...
        <div className="flex items-center gap-1 border border-[var(--c-border)] rounded-lg p-0.5 bg-[var(--c-hover)] shrink-0">
          {viewButtons.map(btn => (
            <button key={btn.id} type="button" onClick={() => setViewMode(btn.id)} aria-pressed={viewMode === btn.id} title={btn.label}
              className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-md text-[12px] font-medium transition-colors border-none cursor-pointer font-[inherit] ${viewMode === btn.id ? 'bg-[var(--c-bg)] text-[var(--c-text)] shadow-sm' : 'bg-transparent text-[var(--c-muted)] hover:text-[var(--c-text-sub)]'}`}>
              {btn.icon}
        ...
        <button type="button" onClick={() => { setForm({ name: '', status: 'planificado', start_date: '', end_date: '' }); setFormError(''); setModalOpen(true); }}
      ...
      {!loading && cycles.length === 0 && (
        <div className="flex flex-col items-center gap-3 py-16 text-center">
          ...
          <p className="text-sm text-[var(--c-muted)]">No hay cycles en este proyecto. Crea el primero.</p>
        </div>
      )}
      ...
      {!loading && cycles.length > 0 && filteredCycles.length === 0 && (
        <div className="flex flex-col items-center gap-3 py-16 text-center">
          ...
          <button type="button" onClick={() => setFilterStatus([])} className="text-[12px] text-[var(--c-accent)] cursor-pointer bg-transparent border-none font-[inherit] hover:underline">Limpiar filtros</button>
        </div>
      )}
          ...
          {filteredGroups.map(group => (
            <section key={group.status}>
              <div className="flex items-center gap-2 mb-3">
                <span className="text-[10px] font-bold uppercase tracking-widest rounded-full px-2 py-0.5"
                  style={{ color: `var(--c-cycle-${sKey(group.status)})`, background: `var(--c-cycle-${sKey(group.status)}-bg)` }}>
                  {STATUS_LABEL[group.status]}
                </span>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
                {group.cycles.map(c => (
                  <CycleCard key={c.id} cycle={c}
                    ...
                    onStatusChange={status => { apiPatch(`/cycles/${c.id}`, { status }).catch(console.error); setAllCycles(prev => prev.map(x => x.id === c.id ? { ...x, status } : x)); }}
      ...
      <Modal open={!!manageId} onClose={() => setManageId(null)} title="Agregar tareas al cycle">
        {manageId && (() => {
          const cycle = allCycles.find(c => c.id === manageId);
          ...
          const filtered = available.filter(t => t.title.toLowerCase().includes(searchLow) || t.identifier.toLowerCase().includes(searchLow));
          const epicTotal = new Map<string, number>();
          const epicInC   = new Map<string, number>();
          for (const t of allProjTasks) {
            if (t.epic_id) { epicTotal.set(t.epic_id, (epicTotal.get(t.epic_id) ?? 0) + 1); if (inCycle.has(t.id)) epicInC.set(t.epic_id, (epicInC.get(t.epic_id) ?? 0) + 1); }
          }
          const availableEpics = projEpics.filter(ep => { const total = epicTotal.get(ep.id) ?? 0; return total === 0 || (epicInC.get(ep.id) ?? 0) < total; });
          return (
            <div className="flex flex-col gap-0">
              {inCycle.size > 0 && (
                <div className="flex items-center gap-1.5 mb-3 px-0.5">
                  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden="true" className="text-[var(--c-cycle-active)] shrink-0"><polyline points="20 6 9 17 4 12"/></svg>
                  <span className="text-[12px] text-[var(--c-text-sub)]">{inCycle.size} tarea{inCycle.size !== 1 ? 's' : ''} ya {inCycle.size === 1 ? 'agregada' : 'agregadas'} a este cycle</span>
                </div>
              )}
              <div className="flex gap-0 border-b border-[var(--c-border)] mb-4">
                {([{ id: 'tasks' as const, label: 'Tareas', count: available.length }, { id: 'epics' as const, label: 'Por épica', count: availableEpics.length }]).map(tab => (
                  <button key={tab.id} type="button" onClick={() => setManageTab(tab.id)}
                    className={`flex items-center gap-1.5 text-[13px] px-3 py-2 border-b-2 -mb-px transition-colors bg-transparent border-x-0 border-t-0 cursor-pointer font-[inherit] ${manageTab === tab.id ? 'border-b-[var(--c-text)] text-[var(--c-text)] font-semibold' : 'border-b-transparent text-[var(--c-text-sub)] hover:text-[var(--c-text)]'}`}>
                    {tab.label}
                    {tab.count > 0 && <span className="text-[11px] font-semibold rounded-full px-1.5 py-0.5 leading-none bg-[var(--c-hover)] text-[var(--c-muted)]">{tab.count}</span>}
                  </button>
                ))}
              </div>
              {manageTab === 'tasks' && (
                <div className="flex flex-col gap-3">
                  <div className="relative">
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden="true" className="absolute left-3 top-1/2 -translate-y-1/2 text-[var(--c-muted)] pointer-events-none"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
                    <input type="text" placeholder="Buscar por título o ID…" value={taskSearch} onChange={e => setTaskSearch(e.target.value)}
                  ...
                  {filtered.length === 0 ? (
                    <div className="py-6 flex flex-col items-center gap-2 text-center">
                      <p className="text-[13px] text-[var(--c-muted)]">{available.length === 0 ? 'Todas las tareas del proyecto ya están en este cycle' : 'Sin resultados para esa búsqueda'}</p>
                    </div>
                  ) : (
                        ...
                        <li key={t.id} className="flex items-center gap-3 py-2.5">
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                              ...
                              {t.epic_name && <span className="text-[10px] bg-[var(--c-hover)] text-[var(--c-text-sub)] rounded-md px-1.5 py-0.5 border border-[var(--c-line)]">{t.epic_name}</span>}
                            </div>
                          </div>
                          <button type="button" onClick={() => addTaskToCycle(manageId, t)}
              ...
              {manageTab === 'epics' && (
                <div className="flex flex-col gap-2">
                  {availableEpics.length === 0 ? (
                    <div className="py-6 flex flex-col items-center gap-2 text-center">
                      <p className="text-[13px] text-[var(--c-muted)]">{projEpics.length === 0 ? 'No hay épicas activas en este proyecto' : 'Todas las épicas ya están en este cycle'}</p>
                    </div>
                  ) : (
                          ...
                          <div className="flex items-start justify-between gap-3">
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-2">
                                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden="true" className="text-[var(--c-text-sub)] shrink-0"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>
                                <span className="text-[13px] font-semibold text-[var(--c-text)] leading-snug">{ep.name}</span>
                              </div>
                              <p className="text-[11px] text-[var(--c-muted)] mt-1 pl-5">Agrega todas las tareas de esta épica al cycle</p>
                            </div>
                            <button type="button" onClick={() => cycle && addEpicToCycle(manageId, ep.id, cycle.project_code)}
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title="Nuevo cycle">
        <form onSubmit={handleCreate} noValidate className="flex flex-col gap-4">
          <div className="flex flex-col gap-1">
            ...
            <input type="text" placeholder="Ej. Sprint 1" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))}
              ...
              {(['planificado', 'activo', 'completado'] as CycleSummary['status'][]).map(s => {
                ...
                return (
                  <button key={s} type="button" onClick={() => setForm(f => ({ ...f, status: s }))}
                    ...
                    style={active ? { background: `var(--c-cycle-${sk}-bg)`, color: `var(--c-cycle-${sk})`, borderColor: `var(--c-cycle-${sk})` } : { background: 'transparent', color: 'var(--c-text-sub)', borderColor: 'var(--c-border)' }}>
                    <span className="w-1.5 h-1.5 rounded-full shrink-0" style={{ background: active ? `var(--c-cycle-${sk})` : 'var(--c-muted)' }} />
                    {STATUS_LABEL[s]}
                  </button>
                );
              })}
      ...
      <ConfirmModal open={pendingRemove !== null} title="Quitar tarea del cycle" message="La tarea se quitará de este cycle pero no se eliminará." confirmLabel="Quitar"
      ...
      <Modal open={!!editCycle} onClose={() => setEditCycle(null)} title="Editar cycle">
        <form onSubmit={handleEdit} noValidate className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-1">
            <label className="text-xs font-semibold text-[var(--c-text-sub)] uppercase tracking-wide">Estado</label>
            <select value={editForm.status} onChange={e => setEditForm(f => ({ ...f, status: e.target.value as CycleSummary['status'] }))}
              className="w-full border border-[var(--c-border)] rounded-lg px-3 py-2 text-sm bg-[var(--c-bg)] text-[var(--c-text)] focus:outline-none focus:border-[var(--c-text-sub)] transition-colors">
          ...
          <div className="flex justify-end gap-2 pt-1">
            <button type="button" onClick={() => setEditCycle(null)}
              className="text-sm font-semibold text-[var(--c-text-sub)] border border-[var(--c-border)] rounded-lg px-4 py-2 bg-transparent hover:bg-[var(--c-hover)] transition-colors cursor-pointer font-[inherit]">
              Cancelar
            </button>
      ...
      <ConfirmModal open={pendingDeleteId !== null} title="Eliminar cycle" message="Se eliminará el cycle y sus tareas se desvincularán. Esta acción no se puede deshacer." confirmLabel="Eliminar"
        onConfirm={handleDelete} onCancel={() => setPendingDeleteId(null)} />
    </div>
  );
}
